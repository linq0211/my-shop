  ...
  <uni-popup ref="popup" type="bottom" background-color="#fff">
    <view class="popup-root">
      ...
      <view class="footer">
        ...
        <view class="button primary" @tap="onConfirm">确认</view>
      </view>
    </view>
  </uni-popup>
</template>
...
import { OrderState, orderStateList } from '@/services/modules/constants'
import {
  deleteMemberOrder,
  getMemberOrderById,
...
import { ref } from 'vue'
import pageSkeleton from './cpns/page-skeleton.vue'
...
const fetchOrderAction = async () => {
  ...
  if (
    ...
    await getMemberOrderLogisticsData()
  }
}
...
const getMemberOrderLogisticsData = async () => {
  ...
  logistList.value = res.result.list
}
...
const onPay = async () => {
  if (isDev) {
    // 开发环境模拟支付
    const res = await getPayMock({ orderId: query.id })
  } else {
    // 生产环境获取微信支付参数
    const res = await getPayWxPay({ orderId: query.id })
    // 发起微信支付
    await wx.requestPayment(res.result)
  }
  uni.redirectTo({ url: `/pageOrder/payment/payment?id=${query.id}` })
}
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        ...
        uni.redirectTo({ url: '/pageOrder/list/list' })
      }
    }
  })
}
...
const onConfirm = async () => {
  if (!reason.value) return uni.showToast({ icon: 'error', title: '请选择取消原因' })
  await getMemberOrderCancelById(query.id, { cancelReason: reason.value })
  popup.value?.close?.()
  uni.navigateBack()
}
</script>
