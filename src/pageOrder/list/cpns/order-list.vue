<template>
  <scroll-view scroll-y class="orders" @scrolltolower="fetchOrderAction">
    <view class="card" v-for="order in orderList" :key="order.id">
      ...
      <block v-for="sku in order.skus" :key="sku.id">
        <navigator
          class="goods"
          :url="`/pageOrder/detail/detail?id=${order.id}`"
          hover-class="none"
        >
          <view class="cover">
      ...
      <view class="action">
        ...
        <template v-else>
          <navigator
            ...
            :url="`/pageOrder/create/create?orderId=${order.id}`"
            hover-class="none"
          >
            再次购买
          </navigator>
...
const orderListParams: Required<OrderListParams> = {
  page: 1,
  pageSize: 10,
...
const fetchOrderAction = async () => {
  ...
  const res = await getMemberOrder(orderListParams)
  // 发送请求后，重置标记
  isLoading.value = false
  // 数组追加
  orderList.value.push(...res.result.items)
  // 分页条件
  if (orderListParams.page < res.result.pages) {
    // 页码累加
    orderListParams.page++
  } else {
    // 分页已结束
    isFinish.value = true
  }
...
const onPay = async (id: string) => {
  if (import.meta.env.DEV) {
    // 开发环境模拟支付
    const res = await getPayMock({ orderId: id })
  } else {
    // 生产环境获取微信支付参数
    const res = await getPayWxPay({ orderId: id })
    // 发起微信支付
    await wx.requestPayment(res.result)
  }
  uni.showToast({ icon: 'success', title: '支付成功' })
  const order = orderList.value.find((i) => i.id === id)
